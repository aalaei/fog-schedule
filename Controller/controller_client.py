...
class ControllerClient(protocol.Protocol):
    def __init__(self, chosen_task, task_id, _difficulty_level, set_communication_demand, fog_id):
    ...
    def dataReceived(self, data):
        if self.is_info_known is False:
            ...
            if verify_ans(self.chosen_task, data, self._difficulty_level):
                print(Fore.GREEN +
                      "ans is verified from {}/ taken Time={:.4f}+{:.4f}={:.4f}s,  R={:.2f} MBytes/s, Service Time={:.4f}s".format(
                          self.fog_id,
...
class ControllerClientFactory(ClFactory):
    ...
    def buildProtocol(self, addr):
        return ControllerClient(self.chosen_task, self.task_id, self._difficulty_level, self.set_communication_demand, self.fog_id)
